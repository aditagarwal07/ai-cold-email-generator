import os
import requests
import streamlit as st
from google import genai

st.set_page_config(page_title="AI Cold Email Generator", page_icon="✉️")

st.title("✉️ AI Cold Email & Pitch Generator")
st.write("Generate high-converting cold outreach emails using AI.")

# Retrieve key safely from Streamlit Secrets or Environment
api_key = st.secrets.get("GEMINI_API_KEY") or os.environ.get("GEMINI_API_KEY")

st.sidebar.header("System Status")
if not api_key:
    st.sidebar.error("⚠️ API Key Missing! Please add GEMINI_API_KEY to Secrets.")
    st.stop()
else:
    st.sidebar.success("✅ Connected to Gemini API")

# Initialize session state for holding generated text
if "generated_pitch" not in st.session_state:
    st.session_state["generated_pitch"] = ""

with st.form("email_form"):
    st.subheader("Campaign Details")
    user_offer = st.text_input("Your Product/Service", placeholder="e.g., Web Design for E-commerce")
    target_client = st.text_input("Target Client", placeholder="e.g., Marketing Manager")
    client_pain_point = st.text_area("Pain Point Solved", placeholder="e.g., Low conversion rates")
    email_tone = st.selectbox("Tone", ["Short & Punchy", "Problem-Solver", "Formal"])
    submit = st.form_submit_button("🚀 Generate 3 Email Drafts")

if submit:
    if not user_offer or not target_client:
        st.warning("Please fill in your offer and target client.")
    else:
        with st.spinner("Generating cold emails..."):
            try:
                client = genai.Client(api_key=api_key)
                prompt = f"""
                Act as a master copywriter. Write 3 distinct cold email options for:
                - Service: {user_offer}
                - Recipient: {target_client}
                - Pain Point: {client_pain_point}
                - Tone: {email_tone}
                
                Provide a Subject Line, Body, and Call to Action for Option 1, Option 2, and Option 3.
                """
                response = client.models.generate_content(
                    model='gemini-3.6-flash',
                    contents=prompt
                )
                
                # Save generated text in session state
                st.session_state["generated_pitch"] = response.text
                st.success("Your Emails are Ready!")
            except Exception as e:
                st.error(f"Error: {str(e)}")

# Display generated pitch if present
if st.session_state["generated_pitch"]:
    st.markdown(st.session_state["generated_pitch"])

# --- Make.com Email Delivery Integration ---
st.divider()
st.subheader("📧 Receive Draft in Your Inbox")

# 1. Text Input Field for the user's email address
user_email = st.text_input("Enter your email address:", placeholder="name@example.com")

# 2. Webhook URL
make_webhook_url = "https://hook.eu1.make.com/s51yp5visu3qc5u00vabubovwpf6b6xx"

# 3. Action Button
if st.button("🚀 Send Pitch to My Email"):
    if user_email:
        if not st.session_state["generated_pitch"]:
            st.warning("Please generate email drafts above first before sending.")
        else:
            payload = {
                "recipient_email": user_email,
                "email_subject": "Your AI-Generated Cold Email Pitch",
                "email_body": st.session_state["generated_pitch"]
            }
            
            try:
                response = requests.post(make_webhook_url, json=payload)
                if response.status_code == 200:
                    st.success("Draft sent successfully! Check your inbox.")
                else:
                    st.error("Failed to send. Please verify the Webhook URL.")
            except Exception as e:
                st.error(f"Error connecting to Make.com: {e}")
    else:
        st.warning("Please enter a valid email address before sending.")
